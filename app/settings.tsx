...
export default function SettingsScreen() {
  ...
  return <View style={styles.screen}><ScrollView contentContainerStyle={[styles.content, { paddingBottom: 110 + insets.bottom }]}>
    ...
    <Section title={t('settings.mapData')}><ExternalRow icon="map" title="OpenFreeMap" description={t('settings.mapAttribution')} onPress={() => void Linking.openURL('https://openfreemap.org/')} /></Section>
...
function ExternalRow({ icon, title, description, onPress }: { icon: keyof typeof MaterialIcons.glyphMap; title: string; description: string; onPress: () => void }) { return <Pressable style={styles.row} onPress={onPress} accessibilityRole="link"><MaterialIcons name={icon} size={22} color="#5F6368" /><View style={styles.rowCopy}><Text style={styles.rowTitle}>{title}</Text><Text style={styles.rowDescription}>{description}</Text></View><MaterialIcons name="open-in-new" size={18} color="#5F6368" /></Pressable>; }
