...
function zoneBoundsForViewport(size: Size, viewport: Viewport): [number, number, number, number] {
  const topLeft = screenToGeo(0, 0, size, viewport);
  const bottomRight = screenToGeo(size.width, size.height, size, viewport);
  return [
    clamp(topLeft.longitude, -180, 180),
    clamp(bottomRight.latitude, -85, 85),
    clamp(bottomRight.longitude, -180, 180),
    clamp(topLeft.latitude, -85, 85),
  ];
}

function viewportForZone(zone: GribZone, size: Size): Viewport {
  const longitudeSpan = Math.max(0.1, zone.rightlon - zone.leftlon);
  const latitudeSpan = Math.max(0.1, zone.toplat - zone.bottomlat);
  const baseScale = Math.min(size.width / 360, size.height / 170);
  const fitScale = Math.min(size.width / longitudeSpan, size.height / latitudeSpan);
  return constrainViewport({
    centerLon: (zone.leftlon + zone.rightlon) / 2,
    centerLat: (zone.bottomlat + zone.toplat) / 2,
    zoom: clamp(fitScale / baseScale, MIN_ZOOM, MAX_ZOOM),
  });
}
...
export function EmbeddedZonePickerMap({ zone, focusRequest = 0, onChange }: ZonePickerMapProps) {
  ...
  const updateViewport = useCallback((next: Viewport) => {
    ...
    if (size.width > 1 && size.height > 1) {
      onChange(zoneFromVisibleBounds(zoneBoundsForViewport(size, constrained), language));
    }
  }, [language, onChange, size]);

  useEffect(() => {
    if (size.width <= 1 || size.height <= 1) return;
    if (initialized.current && lastFocusRequest.current === focusRequest) return;
    initialized.current = true;
    lastFocusRequest.current = focusRequest;
    updateViewport(viewportForZone(zone, size));
  }, [focusRequest, size, updateViewport, zone]);
